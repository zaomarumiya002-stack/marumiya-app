...
# --- 📦 在庫・スケジュール ---
    ...
    with t2:
        ...
        st.download_button("🖨️ 週間カレンダーExcel出力", data=pd.DataFrame(cal_data).to_csv(index=False, encoding="utf-8-sig"), file_name=f"予定_{today.strftime('%Y%m%d')}.csv", type="primary", use_container_width=True)
        MAX_CS = 500
