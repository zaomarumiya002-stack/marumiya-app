...
p_sum = {}
pk_mu = pk_m.drop_duplicates(subset=["資材名"]) if not pk_m.empty else pd.DataFrame(columns=["資材名","品番","規格","仕入先","保管場所","単位","初期在庫","発注点","発注リードタイム"])
if not pk_mu.empty:
    for _, r in pk_mu.iterrows(): p_sum[r["資材名"]] = {"品番":str(r.get("品番","")), "規格":str(r.get("規格","")), "仕入先":str(r.get("仕入先","")), "保管場所":str(r.get("保管場所","")), "単位":str(r.get("単位","")), "期首在庫":to_int(r.get("初期在庫",0)), "発注点":to_int(r.get("発注点",0)), "発注リードタイム":to_int(r.get("発注リードタイム",7)), "入庫":0, "出庫":0, "現在庫":0}
if not pk_l.empty:
...
if pg == "📋 受注登録":
    ...
    m_add = st.empty()
    if st.session_state.get("msg_order_add"): m_add.success(st.session_state.msg_order_add); st.session_state.msg_order_add = None
    ...
    if st.button("✅ 受注を登録", type="primary", use_container_width=True):
        if not prod or not qty or to_int(qty)<1: m_add.error("⚠️ 製品・ケース数は必須です。")
            ...
            st.session_state.msg_order_add = f"✨ 登録完了: {prod} ({to_int(qty)}cs) {format_date_jp(od) if od else '日付未定'}"; st.rerun()
...
# ─────────────────────────────────────────────
    ...
    m_add = st.empty()
    if st.session_state.get("msg_manu_add"): m_add.success(st.session_state.msg_manu_add); st.session_state.msg_manu_add = None

    if st.button("➕ 製造データを記録", type="primary", use_container_width=True):
        if not pm or not mq: m_add.error("⚠️ 製品・数量は必須")
        else:
            ...
            st.session_state.msg_manu_add = f"✨ 製造登録完了: {pm} ({to_int(mq)}cs)"
            st.rerun()
...
# ─────────────────────────────────────────────
    ...
    with tp1:
        if pk_m.empty: st.warning("マスタに資材がありません。")
            ...
            no_lt = [r["資材名"] for _, r in pk_mu.iterrows() if to_int(r.get("発注リードタイム", 0)) == 0]
            if no_lt: st.warning(f"⚠️ **LT未設定の資材:** {' / '.join(no_lt)}")
            ...
            for pn, du in pf.items():
                ...
                oa.append({"資材名":pn,"現在庫":ci,"発注点":pt,"LT":lt,"90日消費":int(cu),"発注推奨日":od.strftime("%Y/%m/%d (%a)") if od else "―","到達日":ro_d.strftime("%Y/%m/%d") if ro_d else "なし","枯渇予測":z_d.strftime("%Y/%m/%d (%a)") if z_d else "―","予測在庫":f"{io:,}" if io is not None else "―","緊急度":urg,"_s":dl,"_c":uc,"_b":bc})
            ...
            spg = st.selectbox("グラフ表示", options=[r["資材名"] for _, r in pk_mu.iterrows()])
            if spg:
    ...
    with tp3:
        ...
        c1,c2 = st.columns([1.5,2.5]); s_pk = c1.text_input("🔍 検索"); f_pk = [p for p in pk_mu["資材名"].tolist() if s_pk in p] if s_pk else pk_mu["資材名"].tolist()
        ...
        m_add = st.empty()
        if st.session_state.get("msg_pack_add"): m_add.success(st.session_state.msg_pack_add); st.session_state.msg_pack_add = None
        ...
        if st.button("➕ 登録", type="primary", use_container_width=True):
            if not sl_pk or pq is None: m_add.error("⚠️ 必須")
                ...
                if lq>0:
                    ...
                    st.session_state.msg_pack_add = f"✨ 資材ログ登録: {sl_pk} ({fpt} {lq})"
                    st.rerun()
        ...
        for i in range(7):
            ...
            if not odf.empty:
                for _,r in odf[pd.to_datetime(odf["納品予定日"],errors='coerce').dt.normalize()==d2].iterrows():
                    ...
                    sod = fs.get(p2,{}).get(d2,0)
                    is_chk = r.get("荷姿チェック") in [True,"TRUE"]
                    is_irr = r.get("不良廃棄フラグ") in [True,"TRUE"]
                    qh, bg, bc = "", "#F0F7FF", "#2563EB"
                    if is_chk: qh, bg, bc = f'<span style="text-decoration:line-through;">{qty2}cs</span>', "#D1FAE5", "#059669"
                    ...
                    oh+=f'<div style="background:{bg};border-left:4px solid {bc};padding:6px;margin-bottom:4px;"><b>{r["顧客名"]}: {format_name(p2)}</b><span style="float:right;">{qh}</span></div>'
