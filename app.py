"""
丸実屋 受注・製造・在庫管理アプリ (クラウド・スプレッドシート対応版)
=========================================
修正版: 複数ユーザー同時利用・Streamlit Cloud デプロイ対応
"""
...
/* フォント: クラス名に依存しない確実な指定 */
html, body, p, div, span, li, td, th, label, input, select, textarea, button {
    font-family: 'Noto Sans JP', sans-serif !important;
}
body { background-color: var(--clr-bg) !important; color: var(--clr-text) !important; }
...
[data-testid="stSidebar"] div[role="radiogroup"] label {
    ...
    margin-bottom: 4px !important; transition: background 0.2s, color 0.2s; cursor: pointer;
}
[data-testid="stSidebar"] div[role="radiogroup"] label:hover { background: rgba(255,255,255,0.1) !important; }
[data-testid="stSidebar"] div[role="radiogroup"] label:has(input:checked) {
    background: #0e8c5a !important; box-shadow: 0 4px 6px rgba(0,0,0,0.1);
}
...
div.stRadio > div[role="radiogroup"] > label {
    background-color: #ffffff !important; border: 1.5px solid #d0d7e3 !important;
    border-radius: 20px !important; padding: 6px 14px !important; margin: 0 !important;
...
/* フォームラベル */
label[data-testid="stWidgetLabel"] p { font-size: 13px !important; font-weight: 700 !important; color: #1c2a3a !important; }
...
def get_sheet():
    """毎回確実に最新の接続を取得する（タイムアウト対策）"""
    try:
        client = init_connection()
        return client.open_by_url(st.secrets["spreadsheet_url"])
    except Exception:
        # 接続が切れていたらキャッシュをクリアして再接続
...
# ─────────────────────────────────────────────
# データアクセス関数 (スプレッドシート読み書き)
# 【修正】 キャッシュTTLを30秒に延ばし、複数ユーザー競合を軽減
#          save後は clear() ではなく rerun() で自然に再取得
# ─────────────────────────────────────────────
def get_empty_df(sheet_name):
    if sheet_name == "orders":
        return pd.DataFrame(columns=["ID", "納品予定日", "顧客名", "大カテゴリ", "製品名", "ケース数", "登録日時"])
    elif sheet_name == "manufactures":
        return pd.DataFrame(columns=["ID", "製造予定日", "備考", "大カテゴリ", "製品名", "ケース数", "登録日時"])
    elif sheet_name == "master":
        return pd.DataFrame(columns=["大カテゴリ", "製品名", "初期在庫数"])
    elif sheet_name == "customers":
        return pd.DataFrame(columns=["顧客名", "ふりがな"])
    return pd.DataFrame()

# 【修正】 キャッシュをセッション単位に変更
#   - @st.cache_data はプロセス共有のため、複数ユーザーで古いデータを掴み続ける問題がある
#   - TTLを0にして毎回読む代わりに、session_state でユーザー毎にキャッシュする
#   - マスタ等の変化が少ないデータは cache_data(ttl=30) で適度にキャッシュ
@st.cache_data(ttl=30, show_spinner=False)
def _load_data_cached(sheet_name: str, _cache_bust: int):
    """内部キャッシュ関数。_cache_bust を変えると強制再読み込み。"""
    try:
        sh = get_sheet()
...
def _get_cache_bust(sheet_name: str) -> int:
    """セッション内のキャッシュバストカウンターを返す"""
    key = f"cache_bust_{sheet_name}"
    if key not in st.session_state:
        st.session_state[key] = 0
    return st.session_state[key]

def _increment_cache_bust(sheet_name: str):
    """保存後にキャッシュバストカウンターを増やして次回再読み込みを強制する"""
    key = f"cache_bust_{sheet_name}"
    st.session_state[key] = st.session_state.get(key, 0) + 1

def load_data(sheet_name: str) -> pd.DataFrame:
    return _load_data_cached(sheet_name, _get_cache_bust(sheet_name))

def save_data(sheet_name: str, df: pd.DataFrame):
    """
    【修正】 競合対策:
      - ws.clear() + ws.update() を batch_update で1回のAPIコールに
      - 保存後はセッション単位のキャッシュバストカウンターをインクリメント
    """
    max_retries = 3
    for attempt in range(max_retries):
        try:
            ...
            values = [df_str.columns.values.tolist()] + df_str.values.tolist()
            # 【修正】 clear + update を batch で最小化（競合ウィンドウを短縮）
            ws.clear()
            ws.update(values=values, range_name="A1")
            # セッション側のキャッシュを無効化
            _increment_cache_bust(sheet_name)
            return
...
# マスタ定義と初期データ
# ─────────────────────────────────────────────
CATEGORIES = ["平こん", "つきこん", "糸こん・しらたき", "三角こん", "玉こん",
              "ダイスこん", "短冊", "国産", "ちぎりこん", "大黒屋", "かねこ", "冷凍耐性", "その他"]
CAT_ICONS = {
    "平こん": "🟫", "つきこん": "🍝", "糸こん・しらたき": "🍜", "三角こん": "🔺",
    "玉こん": "🟤", "ダイスこん": "🎲", "短冊": "🏷️", "国産": "🇯🇵",
    "ちぎりこん": "🤲", "大黒屋": "🏮", "かねこ": "🏭", "冷凍耐性": "❄️", "その他": "📦"
}
CAT_COLORS = {
    "平こん": "#8b5a2b", "つきこん": "#0e8c5a", "糸こん・しらたき": "#7c3db8",
    "三角こん": "#d97706", "玉こん": "#6b4226", "ダイスこん": "#0e7c8c",
    "短冊": "#1a6fc4", "国産": "#c41a3a", "ちぎりこん": "#d96606",
    "大黒屋": "#333333", "かねこ": "#2563a8", "冷凍耐性": "#4a90e2", "その他": "#6b7280"
}
...
def load_master() -> pd.DataFrame:
    ...
    if df.empty:
        default_products = {
            "つきこん": ["つきこん（黒）5×60 1kg", "つきこん（白）5×60 1kg", "つきこん（黒）5×30 1kg", "つきこん（白）5×30 1kg", "つきこん（黒）5×40 1kg", "つきこん（黒）170g", "つきこん（白）170g", "つきこん（白）3×60 1kg", "つきこん（黒）5×80 1kg", "つきこん（黒）500g", "つきこん（白）5×60 500g", "（特）つきこん（白）5kg×2", "神栄　つきこん（黒）5×60 1kg", "① 0.25 つきこん（黒）5×40 1kg", "サンマリ つきこん 200g", "フジッコ つきこん（黒）T5×80 5kg", "海藻粉2倍 つきこん（黒）3×40 5kg×2"],
            "平こん": ["（特）板こん（白）5kg×2", "板こん（白）1kg", "板こん（黒）1kg", "板こん（白）250g", "板こん（黒）250g", "板こん（黒）500g", "板こん（白）500g", "四角こん（白）1kg", "サンマリ 平こん（白）250g", "サンマリ 平こん（黒）250g"],
            "糸こん・しらたき": ["糸こん（黒）10cm 1kg", "糸こん（白）10cm 1kg", "糸こん（黒）500g", "糸こん（白）5cm 1kg", "糸こん（白）5cm 5kg", "糸こん（黒）5cm 1kg", "しらたき 5cm 1kg", "しらたき 10cm 1kg", "しらたき 10cm 500g", "しらたきエンドレス 200g", "しらたき 10cm 1kg 6ヶ月", "しらたきエンドレス 500g", "神栄　しらたき 1kg", "① 糸こん（白）10cm 1kg", "サンマリ 糸こん（白）200g", "白滝5cm 5kg×2"],
            "三角こん": ["三角（白）小 6g", "三角（黒）小 10g", "三角（黒）小 6g", "三角（黒）大 75g", "三角（黒）大43g 1kg", "三角（白）大 1kg", "三角（黒）中 1kg", "三角（白）中 1kg", "三角（黒）鹿の子 1kg", "三角（黒）30g 1kg", "神栄　三角（黒）中 1kg", "ガーリック三角鹿の子 30g"],
            "玉こん": ["味付玉こん40ヶ入", "味付玉こん6ヶ入", "味付玉こん無添加 6ヶ入", "味付玉こん20ヶ入", "玉こん（黒）30g 1kg", "玉こん（白）30g 1kg", "玉こん（白）30g 500g", "玉こん（黒）18g 1kg", "玉こん（黒）18g 500g", "玉こん（黒）200g （18g）", "玉こん（黒）13g 1kg", "玉こん（白）13g 1kg", "玉こん（白）18g 500g", "玉こん（白）18g 1kg", "クリタ ピリ辛玉こん15個入（昆布入）", "高瀬 ピリ辛玉こん41個入"],
            "ダイスこん": ["神栄　ダイス（黒）1.5cm 1kg", "味付ダイス 一味入1.5cm 1kg×12", "ダイス（黒）25×35×15 1kg", "ダイス（黒）2cm 500g", "ダイス（黒）2cm 1kg", "ダイス（黒）鹿ノ子 37×27×20", "ダイス（白）2cm 1kg", "ダイス（黒）鹿の子 1kg", "ダイス（黒）1.5cm 1kg", "ダイス（白）1.5cm 1kg", "ダイス（黒）20×25×25 1kg", "フジッコ ダイス20（黒）T 5kg", "粒こん（白）2kg×5"],
            "冷凍耐性": ["蔵王ミート　耐冷糸こん（黒）", "AK カットこん耐冷（白）3×30 2kg×5", "高瀬 耐冷チップこん（黒）1kg", "モガミフーズ 耐冷つきこん（黒）3×23 0.25%", "耐冷 短冊（黒）1kg", "耐冷 こんにゃく20×25×10（黒）1kg", "耐冷 糸こん（黒）1kg", "耐冷 糸こん（白）10cm 1kg", "耐冷 つきこん（黒）5×30 1kg", "耐冷 つきこん（白）3×30 1kg", "耐冷 つきこん（黒）5×40 1kg", "耐冷 つきこん（黒）5×60 1kg", "耐冷 糸こん（白）7cm 1kg", "耐冷 糸こん（白）7cm 5kg×2", "耐冷 ちぎり（黒）1kg", "耐冷 つきこん（白）4×140cm 5kg×2", "耐冷 板こん（黒）1kg", "耐冷 ダイス2cm（黒）1kg", "耐冷 三角（黒）10g 1kg", "耐冷 ダイス1.5（黒）1kg", "耐冷 玉こん（黒）10g 1kg", "耐冷 しらたき 5cm 1kg"],
            "短冊": ["短冊（黒）1kg", "短冊 500g", "短冊（白）1kg", "短冊（黒）25×20×10 1kg", "海藻粉2倍 短冊（黒）30×20×5 5kg×2", "ガーリック短冊（黒）20×25×5 2kg×4"],
            "国産": ["国産 つきこん（黒）5×20 1kg", "国産 つきこん（黒）5×60 1kg", "国産 つきこん（黒）8×40 1kg", "国産 板こん（黒）1kg", "国産 糸こん（黒） 1kg", "国産 三角（黒）中 1kg", "国産 三角（黒）小 1kg", "国産 玉こん（黒）18g 1kg", "国産 ダイス（黒）1.5cm 1kg", "国産 ダイス（黒）2cm 1kg"],
            "ちぎりこん": ["ちぎり（黒）1kg", "味付ちぎり", "ゆで太郎 ちぎり（黒）1kg 緑", "ちぎり（黒）250g"],
            "大黒屋": ["大黒屋 つきこん（黒）6×50 1kg", "大黒屋 つきこん（黒）5×30 1kg", "大黒屋 つきこん 400g", "大黒屋 徳用しらたき 1kg", "大黒屋 カットしらたき 5cm 1kg", "大黒屋 ちぎり 400g", "大黒屋 カットスライス 短冊25×20×5"],
            "かねこ": ["かねこ つきこん（黒）5×60 1kg×12", "かねこ つきカット（黒）5×40 1kg×12", "かねこ 平こん（黒）1kg", "かねこ サイコロ（黒）2cm 1kg×12", "かねこ サイコロ大（黒）20×25×25 1kg×12"],
            "その他": ["コラーゲン入りこんにゃくうどん 1kg", "サラダ 1kg", "カットこんにゃく（黒）27×37×20 1kg", "こんにゃく（白）平麺 1kg", "こんにゃく麺（黄）1kg", "にしき食品 チップこん（黒）13×25×5 0.3mmO", "ショクカイ さしみ（白）", "ショクカイ さしみのり（緑）", "ショクカイ サラダ3色", "あさひ食品 12.5×12.3×31", "あさひ食品 20×4×31", "あさひ食品 40×5×31", "うま煮 ホタテ風味", "うま煮 ぴり辛風味"],
        }
        rows = [{"大カテゴリ": cat, "製品名": p, "初期在庫数": 0}
                for cat, products in default_products.items() for p in products]
...
def load_customers() -> pd.DataFrame:
    ...
    if df.empty:
        raw_cust = [
            ("㈱ショクカイ", "しょくかい"), ("㈱大黒屋商店", "だいこくやしょうてん"),
            ("(有)かねこ食品工業", "かねこしょくひんこうぎょう"), ("三幸食品㈱", "さんこうしょくひん"),
            ("㈱石田食品販売", "いしだしょくひんはんばい"), ("松田産業㈱仙台営業所", "まつださんぎょうせんだいえいぎょうしょ"),
            ("㈱ピアット", "ぴあっと"), ("太平物産", "たいへいぶっさん"), ("大阪太平", "おおさかたいへい"),
            ("半田商店", "はんだしょうてん"), ("富山栄養", "とやまえいよう"), ("千葉ｺｰﾌﾟ", "ちばこーぷ"),
            ("日栄物産㈱", "にちえいぶっさん"), ("㈱ｼﾙﾊﾞｰﾗｲﾌ", "しるばーらいふ"), ("神栄㈱", "しんえい"),
            ("㈱ｽｽﾞｶ未来", "すずかみらい"), ("㈱クサマ", "くさま"), ("㈱あさひ食品", "あさひしょくひん"),
            ("㈱山形南生活総合ｾﾝﾀｰ", "やまがたみなみせいかつそうごうせんたー"), ("㈱三和食品", "さんわしょくひん"),
            ("(有)ゆうから", "ゆうから"), ("㈱茨城食品ｶﾞｰﾘｯｸ工業", "いばらきしょくひんがーりっくこうぎょう"),
            ("高瀬物産山形支店", "たかせぶっさんやまがたしてん"), ("高瀬物産㈱弘前支店", "たかせぶっさんひろさきしてん"),
            ("髙瀬物産㈱千葉支店", "たかせぶっさんちばしてん"), ("高瀬物産㈱仙台支店", "たかせぶっさんせんだいしてん"),
            ("髙瀬物産㈱青森支店", "たかせぶっさんあおもりしてん"), ("高瀬物産㈱福島支店", "たかせぶっさんふくしましてん"),
            ("髙瀬物産㈱湘南支店", "たかせぶっさんしょうなんしてん"), ("髙瀬物産㈱練馬支店", "たかせぶっさんねりましてん"),
            ("高瀬物産㈱城東支店", "たかせぶっさんじょうとうしてん"), ("高瀬物産㈱宇都宮支店", "たかせぶっさんうつのみやしてん"),
            ("高瀬物産㈱長野支店", "たかせぶっさんながのしてん"), ("高瀬物産㈱両毛支店", "たかせぶっさんりょうもうしてん"),
            ("高瀬物産㈱三重支店", "たかせぶっさんみえしてん"), ("東海澱粉（株）札幌", "とうかいでんぷんさっぽろ"),
            ("東海澱粉㈱山形営業所", "とうかいでんぷんやまがたえいぎょうしょ"), ("東海澱粉㈱東京支店", "とうかいでんぷんとうきょうしてん"),
            ("東海澱粉㈱福岡", "とうかいでんぷんふくおか"), ("東海澱粉㈱金沢営業所", "とうかいでんぷんかなざわえいぎょうしょ"),
            ("東海澱粉㈱長崎営業", "とうかいでんぷんながさきえいぎょう"), ("ｵｷﾞﾉ食糧株式会社", "おぎのしょくりょう"),
            ("株式会社ﾂｰｴｲﾄ", "つーえいと"), ("㈱ﾏﾙｺ岩村食品団地工場", "まるこいわむらしょくひんだんちこうじょう"),
            ("吉成物産株式会社", "よしなりぶっさん"), ("ｼﾞｰｴﾌｼｰ株式会社", "じーえふしー"),
            ("株式会社ﾆｼﾔﾏ8,020", "にしやま"), ("青和食品株式会社", "せいわしょくひん"),
            ("日進食品株式会社", "にっしんしょくひん"), ("三桜商事株式会社", "さんおうしょうじ"),
            ("石田食品", "いしだしょくひん"), ("三桜商事郡山支店", "さんおうしょうじこおりやましてん"),
            ("ﾏﾙｷｭｰﾌｰｽﾞ株式会社", "まるきゅーふーず"), ("三井物流㈱", "みついぶつりゅう"),
            ("株式会社太堀", "たいぼり"), ("青葉化成株式会社", "あおばかせい"),
            ("青葉化成㈱塩釜営業所", "あおばかせいしおがまえいぎょうしょ"), ("ウジエ仙台", "うじえせんだい"),
            ("株式会社蔵王ﾐｰﾄ", "ざおうみーと"), ("(株)越後ﾊﾑ", "えちごはんむ"),
            ("岩手県学校給食会", "いわてけんがっこうきゅうしょくかい"), ("宮城県学校給食会", "みやぎけんがっこうきゅうしょくかい"),
            ("株式会社ﾏﾙﾄﾓ物産", "まるともぶっさん"), ("ﾕｱｻ･ﾌﾅｼｮｸ株式会社", "ゆあさふなしょく"),
            ("羽黒のうきょう食品加工㈲", "はぐろのうきょうしょくひんかこう"), ("国分北海道株式会社", "こくぶほっかいどう"),
            ("山形県学校給食会", "やまがたけんがっこうきゅうしょくかい"), ("青森県学校給食会", "あおもりけんがっこうきゅうしょくかい"),
            ("㈱ﾗｲﾌﾌﾟﾛﾓｰﾄ", "らいふぷろもーと"), ("株式会社ﾔﾏ食東北営業所", "やましょくとうほくえいぎょうしょ"),
            ("株式会社前川ｲﾝﾀｰﾃｯｸ", "まえかわいんたーてっく"), ("㈱にしむら　庄内支店", "にしむらしょうないしてん"),
            ("㈱にしむら　山形支店", "にしむらやまがたしてん"), ("㈱にしむら　米沢支店", "にしむらよねざわしてん"),
            ("関東砂糖株式会社", "かんとうさとう"), ("国分関信越株式会社", "こくぶかんしんえつ"),
            ("三菱食品株式会社", "みつびししょくひん"), ("東京アグリフーズ", "とうきょうあぐりふーず"),
            ("ｵｲｼｯｸｽ・ﾗ・大地㈱", "おいしっくすらだいち"), ("日興ﾌｰｽﾞ株式会社", "にっこうふーず"),
            ("佐藤株式会社", "さとう"), ("(株)ﾀﾃﾉｺｰﾎﾟﾚｰｼｮﾝ", "たてのこーぽれーしょん"),
            ("国分ﾋﾞｼﾞﾈｽｴｷｽﾊﾟｰﾄ(株)", "こくぶびじねすえきすぱーと"), ("大都魚類株式会社", "だいとぎょるい"),
            ("鳴子観光ﾎﾃﾙ長者原SA(下り線)", "なるこかんこうほてるちょうじゃはら"), ("ｶﾒｲ株式会社", "かめい"),
            ("宮城観光物産", "みやぎかんこうぶっさん"), ("春駒交通㈱", "はるこまこうつう"),
            ("七ヶ宿まちづくり株式会社", "しちかしゅくまちづくり"), ("㈱鬼わさび本舗", "おにわさびほんぽ"),
            ("ｴｺﾌｧｰﾑ蔵王株式会社", "えこふぁーむざおう"), ("まる福㈱", "まるふく"),
            ("宮城県物産振興協会", "みやぎけんぶっさんしんこうきょうかい"), ("株式会社みやぎ蔵王物産", "みやぎざおうぶっさん"),
            ("株式会社仙台百珍味寿や", "せんだいひゃくちんみすや"), ("株式会社カルラ", "かるら"),
        ]
        default_df = pd.DataFrame([{"顧客名": c[0], "ふりがな": c[1]} for c in raw_cust])
        save_data("customers", default_df)
        return default_df
    return df

def save_customers(df: pd.DataFrame): save_data("customers", df)
def load_orders() -> pd.DataFrame: return load_data("orders")
def save_orders(df: pd.DataFrame): save_data("orders", df)
def load_manufactures() -> pd.DataFrame: return load_data("manufactures")
...
# ─────────────────────────────────────────────
# 在庫シミュレーション
# 【修正】 DataFrame を直接 cache_data に渡すと Unhashable でエラー/警告が出るため
#          key となる文字列（タプル）で代替
# ─────────────────────────────────────────────
def simulate_inventory(orders_df, manus_df, master_df, forecast_days=60):
    """キャッシュなし版。呼び出し元で session_state を使ってメモ化する。"""
    o_tmp = orders_df[["納品予定日", "製品名", "ケース数"]].copy().dropna()
    o_tmp = o_tmp.rename(columns={"納品予定日": "日付"})
    o_tmp["変動"] = -o_tmp["ケース数"]

    m_tmp = manus_df[["製造予定日", "製品名", "ケース数"]].copy().dropna()
    m_tmp = m_tmp.rename(columns={"製造予定日": "日付"})
    m_tmp["変動"] = m_tmp["ケース数"]
    ...
    for _, row in master_df.iterrows():
        ...
        current_inv = init_inv + int(past_events["変動"].sum())
...
# ─────────────────────────────────────────────
for _k, _v in [
    ("order_key", 0), ("manu_key", 0),
    ("order_success_msg", ""), ("manu_success_msg", ""),
]:
...
def format_product_name(p_name):
    if pd.isna(p_name):
        return ""
    if "（黒）" in p_name or "黒" in p_name:
        return f"⚫️ {p_name}"
    if "（白）" in p_name or "白" in p_name:
        return f"⚪️ {p_name}"
    return f"📦 {p_name}"
...
with st.sidebar:
    ...
    page = st.radio(
        "メニュー",
        ["📋 受注登録 (出庫)", "🏭 製造登録 (入庫)", "📦 スケジュール・在庫確認",
         "📊 統計・分析", "⚙️ マスタ管理", "📖 使い方ガイド"],
    ...
    if not alerts_df.empty:
        st.markdown(
            f'<div style="background:rgba(217,48,37,0.15);border:1px solid #d93025;border-radius:8px;padding:10px;">'
            f'<div style="color:#ff6b6b;font-weight:900;font-size:14px;margin-bottom:8px;">⚠️ 欠品アラート ({len(alerts_df)}件)</div>',
            unsafe_allow_html=True
        )
        for _, r in alerts_df.iterrows():
            st.markdown(
                f'<div style="font-size:12px;color:#f0f3f8;margin-bottom:4px;">'
                f'<span style="color:#ff6b6b;font-weight:bold;">{r["日付"].strftime("%m/%d")}</span>: '
                f'{r["製品名"]} <span style="color:#ffc107;">(-{r["不足数"]})</span></div>',
                unsafe_allow_html=True
            )
        st.markdown("</div>", unsafe_allow_html=True)
    else:
        st.markdown(
            '<div style="background:rgba(30,140,69,0.15);border:1px solid #1e8c45;border-radius:8px;'
            'padding:10px;text-align:center;color:#4ade80;font-weight:900;font-size:13px;">✅ 欠品の予定はありません</div>',
            unsafe_allow_html=True
        )
...
if page == "📋 受注登録 (出庫)":
    ...
    div.stRadio > div[role="radiogroup"] > label:has(input:checked) {
        ...
        box-shadow: 0 3px 8px rgba(26,111,196,0.3) !important;
    }
    ...
    with tab1:
        ...
        with c1:
            delivery_date = st.date_input("📅 納品予定日 ＊", value=date.today() + timedelta(days=1), min_value=date.today(), key=f"o_date_{okey}")
        with c2:
            cust_opts = ["✏️ 新規・直接入力"] + [
                f"{r['顧客名']} ({r['ふりがな']})" if r['ふりがな'] else r['顧客名']
                for _, r in cust_df.iterrows()
            ]
            selected_cust_opt = st.selectbox("🏢 顧客名を選択 (ふりがな検索可)", cust_opts, key=f"o_cust_sel_{okey}")
            if selected_cust_opt == "✏️ 新規・直接入力":
                customer_name = st.text_input("📝 新規顧客名を入力 (任意)", placeholder="例：〇〇スーパー", key=f"o_cust_txt_{okey}")
            else:
                # 【修正】 ふりがなの括弧を正しく除去
                customer_name = selected_cust_opt.rsplit(" (", 1)[0] if " (" in selected_cust_opt and selected_cust_opt.endswith(")") else selected_cust_opt
        with c3:
            cases = st.number_input("📦 ケース数 ＊", min_value=1, max_value=9999, value=1, step=1, key=f"o_cases_{okey}")
        ...
        with sc1:
            prod_search = st.text_input("🔍 絞り込み検索", placeholder="キーワード...", key=f"o_search_{okey}")
        products_filtered = [p for p in products_all if prod_search in p] if prod_search else products_all
        with sc2:
            if products_filtered and selected_cat:
                selected_product = st.selectbox("📦 製品を選択 ＊", products_filtered, format_func=format_product_name, key=f"o_prod_{okey}")
            else:
                selected_product = ""
                if not selected_cat:
                    st.info("⬆️ まず上のカテゴリボタンを押してください")
        ...
        if st.button("✅ 受注を登録する", type="primary", use_container_width=True):
            if not selected_cat or not selected_product:
                ...
                if c_name != "未指定" and c_name not in cust_df["顧客名"].values:
                    ...
                    save_customers(pd.concat([cust_df, new_cust], ignore_index=True))
                new_row = pd.DataFrame([{
                    "ID": "O-" + str(uuid.uuid4())[:6].upper(),
                    "納品予定日": pd.Timestamp(delivery_date),
                    "顧客名": c_name,
                    "大カテゴリ": selected_cat,
                    "製品名": selected_product,
                    "ケース数": int(cases),
                    "登録日時": pd.Timestamp.now(),
                }])
                # 【修正】 最新データを取得してから concat（他ユーザーの書き込みを取りこぼさない）
                latest_orders = load_orders()
                save_orders(pd.concat([latest_orders, new_row], ignore_index=True))
    ...
    with tab2:
        ...
        if not display_orders.empty and "納品予定日" in display_orders.columns:
            display_orders = display_orders.sort_values("納品予定日", ascending=False).head(50)
        ...
        if edit_target != "➖ 選択してください":
            ...
            # 【修正】 NaT に対して安全なデフォルト値
            date_val = target_row["納品予定日"].date() if pd.notna(target_row["納品予定日"]) else date.today()
            e_date = ec1.date_input("納品予定日", value=date_val)
            cust_opts_edit = ["✏️ 新規・直接入力"] + [
                f"{r['顧客名']} ({r['ふりがな']})" if r['ふりがな'] else r['顧客名']
                for _, r in cust_df.iterrows()
            ]
            ...
            if t_cust != "未指定":
                for i, opt in enumerate(cust_opts_edit):
                    if opt.startswith(t_cust + " (") or opt == t_cust:
                        def_idx = i
                        break
            sel_e_cust = ec2.selectbox("顧客名を選択", cust_opts_edit, index=def_idx)
            if sel_e_cust == "✏️ 新規・直接入力":
                e_cust = ec2.text_input("📝 新規顧客名を入力", value=t_cust if t_cust != "未指定" else "")
            else:
                e_cust = sel_e_cust.rsplit(" (", 1)[0] if " (" in sel_e_cust and sel_e_cust.endswith(")") else sel_e_cust
            e_cases = ec3.number_input("ケース数", value=int(target_row["ケース数"]), min_value=1)
            e_cat = st.selectbox("大カテゴリ", CATEGORIES,
                                 index=CATEGORIES.index(target_row["大カテゴリ"]) if target_row["大カテゴリ"] in CATEGORIES else 0)
            e_mode = st.radio("製品名の入力方法", ["リストから選択", "直接入力 (特注・リスト外)"], horizontal=True)
            if e_mode == "リストから選択":
                p_list = master_df[master_df["大カテゴリ"] == e_cat]["製品名"].tolist()
                if target_row["製品名"] not in p_list:
                    p_list = [target_row["製品名"]] + p_list
                e_prod = st.selectbox("製品名", p_list,
                                      index=p_list.index(target_row["製品名"]) if target_row["製品名"] in p_list else 0,
                                      format_func=format_product_name)
            else:
                e_prod = st.text_input("製品名を直接入力", value=target_row["製品名"])
            ...
            if col_u.button("💾 この内容で更新", type="primary", use_container_width=True):
                ...
                if c_name_edited != "未指定" and c_name_edited not in cust_df["顧客名"].values:
                    save_customers(pd.concat([cust_df, pd.DataFrame([{"顧客名": c_name_edited, "ふりがな": ""}])], ignore_index=True))
                # 【修正】最新を取得してから更新
                latest = load_orders()
                idx = latest[latest["ID"] == edit_target].index
                if len(idx):
                    latest.at[idx[0], "納品予定日"] = pd.Timestamp(e_date)
                    latest.at[idx[0], "顧客名"] = c_name_edited
                    latest.at[idx[0], "大カテゴリ"] = e_cat
                    latest.at[idx[0], "製品名"] = e_prod
                    latest.at[idx[0], "ケース数"] = int(e_cases)
                    save_orders(latest)
                st.rerun()
            if col_d.button("🗑️ このデータを削除", use_container_width=True):
                latest = load_orders()
                save_orders(latest[latest["ID"] != edit_target])
                st.rerun()
        st.markdown("</div>", unsafe_allow_html=True)
...
# ─────────────────────────────────────────────
    ...
    st.markdown("""<style>
    div.stRadio > div[role="radiogroup"] > label:has(input:checked) {
        background-color: var(--clr-manu) !important;
        border-color: var(--clr-manu) !important;
        box-shadow: 0 3px 8px rgba(14,140,90,0.3) !important;
    }
    div.stRadio > div[role="radiogroup"] > label:has(input:checked) p {
        color: #ffffff !important; font-weight: 700 !important;
    }
    ...
    with tab1:
        ...
        with c1:
            manu_date = st.date_input("📅 製造予定日 ＊", value=date.today(), min_value=date.today(), key=f"m_date_{mkey}")
        with c2:
            cases = st.number_input("📦 製造ケース数 ＊", min_value=1, max_value=9999, value=50, step=10, key=f"m_cases_{mkey}")
        with c3:
            note = st.text_input("📝 備考 (ロット等)", placeholder="任意", key=f"m_note_{mkey}")
        ...
        sc1, sc2 = st.columns([1.5, 2.5])
        with sc1:
            prod_search = st.text_input("🔍 絞り込み検索", placeholder="キーワード...", key=f"m_search_{mkey}")
        products_filtered = [p for p in products_all if prod_search in p] if prod_search else products_all
        with sc2:
            if products_filtered and selected_cat:
                selected_product = st.selectbox("📦 製品を選択 ＊", products_filtered, format_func=format_product_name, key=f"m_prod_{mkey}")
            else:
                selected_product = ""
                if not selected_cat:
                    st.info("⬆️ まず上のカテゴリボタンを押してください")
        ...
        if st.button("➕ 製造を登録する", type="primary", use_container_width=True):
            if not selected_cat or not selected_product:
                ...
                # 【修正】最新データを取得してから concat
                latest_manus = load_manufactures()
                save_manufactures(pd.concat([latest_manus, new_row], ignore_index=True))
    ...
    with tab2:
        ...
        display_manus = manus_df.copy()
        if not display_manus.empty and "製造予定日" in display_manus.columns:
            display_manus = display_manus.sort_values("製造予定日", ascending=False).head(50)
        st.dataframe(display_manus, use_container_width=True, hide_index=True)
        st.markdown("</div>", unsafe_allow_html=True)
        ...
        if edit_target != "➖ 選択してください":
            ...
            date_val = target_row["製造予定日"].date() if pd.notna(target_row["製造予定日"]) else date.today()
            e_date = ec1.date_input("製造予定日", value=date_val)
            ...
            col_u, col_d = st.columns(2)
            if col_u.button("💾 この内容で更新", type="primary", use_container_width=True):
                latest = load_manufactures()
                idx = latest[latest["ID"] == edit_target].index
                if len(idx):
                    latest.at[idx[0], "製造予定日"] = pd.Timestamp(e_date)
                    latest.at[idx[0], "備考"] = e_note.strip()
                    latest.at[idx[0], "大カテゴリ"] = e_cat
                    latest.at[idx[0], "製品名"] = e_prod
                    latest.at[idx[0], "ケース数"] = int(e_cases)
                    save_manufactures(latest)
                st.rerun()
            if col_d.button("🗑️ このデータを削除", use_container_width=True):
                latest = load_manufactures()
                save_manufactures(latest[latest["ID"] != edit_target])
                st.rerun()
        st.markdown("</div>", unsafe_allow_html=True)
...
# ─────────────────────────────────────────────
    ...
    if not alerts_df.empty:
        st.markdown('<div class="alert-banner"><div class="alert-banner-icon">🚨</div><div class="alert-banner-text">注意：以下の製品で在庫がマイナス（欠品）になる予定です。至急、製造登録を行ってください。</div></div>', unsafe_allow_html=True)

    tab_inv, tab_alert, tab_sched = st.tabs([
        "📊 在庫推移",
        f"⚠️ アラート ({len(alerts_df)})",
        "📆 直近スケジュール (入出庫)"
    ])

    with tab_inv:
        st.markdown('<div class="card"><div class="card-title">📅 向こう2週間の日別在庫推移</div>'
                    '<div style="font-size:13px;margin-bottom:10px;">※ 数量がマイナスになる日は赤字で表示されます。（初期在庫 ＋ 累積製造 － 累積出荷）</div>',
                    unsafe_allow_html=True)
        display_days = 14
        end_date = pd.Timestamp.today().normalize() + timedelta(days=display_days)
        inv_view = inv_df[inv_df["日付"] <= end_date]
        if not inv_view.empty:
            pivot_df = inv_view.pivot_table(
                index=["大カテゴリ", "製品名"], columns="日付_str", values="在庫数", aggfunc="last"
            ).reset_index()

            def highlight_negative(val):
                if isinstance(val, (int, float)) and val < 0:
                    return "color: #d93025; font-weight: 900; background-color: #fce8e6;"
                return ""
    ...
    with tab_alert:
        ...
        if alerts_df.empty:
            st.success("素晴らしい！現在、欠品予定の製品はありません。")
        else:
    ...
    with tab_sched:
        ...
        max_order = int(orders_df["ケース数"].max()) if not orders_df.empty else 1
        max_manu = int(manus_df["ケース数"].max()) if not manus_df.empty else 1
        ...
        def get_bar_html(val, color_hex, is_plus=True):
            ...
            return (f'<div style="position:relative;background:#e0e6ed;border-radius:4px;height:18px;margin-top:4px;overflow:hidden;">'
                    f'<div style="width:{pct}%;background:{color_hex};height:100%;border-radius:4px;opacity:0.85;"></div>'
                    f'<div style="position:absolute;top:0;right:4px;line-height:18px;font-size:11px;font-weight:900;color:#1c2a3a;">{sign}{val}</div></div>')

        body = '<tr><td class="col-label" style="color:var(--clr-manu);">🏭 製造予定<br><span style="font-size:10px">(入庫)</span></td>'
        for d in day_list:
            # 【修正】日付比較は normalize した Timestamp 同士で行う
            day_manus = manus_df[manus_df["製造予定日"].dt.normalize() == d] if not manus_df.empty else pd.DataFrame()
            cell = ""
        ...
        for d in day_list:
            day_orders = orders_df[orders_df["納品予定日"].dt.normalize() == d] if not orders_df.empty else pd.DataFrame()
            cell = ""
...
# ─────────────────────────────────────────────
    ...
    if orders_df.empty:
        st.warning("分析する受注データがありません。")
    else:
        ana_df = orders_df.dropna(subset=["納品予定日"]).copy()
        ...
        <div style="display:flex;gap:12px;flex-wrap:wrap;margin-bottom:18px;">
          <div class="card" style="flex:1;min-width:150px;text-align:center;border-left:5px solid var(--clr-primary);">
            <div style="font-size:11px;font-weight:bold;color:var(--clr-subtext);">総受注件数</div>
            <div style="font-size:24px;font-weight:900;">{total_orders:,} <span style="font-size:12px;">件</span></div>
          </div>
          <div class="card" style="flex:1;min-width:150px;text-align:center;border-left:5px solid var(--clr-accent);">
            <div style="font-size:11px;font-weight:bold;color:var(--clr-subtext);">総出荷ケース数</div>
            <div style="font-size:24px;font-weight:900;">{total_cases:,} <span style="font-size:12px;">ケース</span></div>
          </div>
          <div class="card" style="flex:1;min-width:150px;text-align:center;border-left:5px solid var(--clr-success);">
            <div style="font-size:11px;font-weight:bold;color:var(--clr-subtext);">取引顧客数</div>
            <div style="font-size:24px;font-weight:900;">{total_custs} <span style="font-size:12px;">社</span></div>
          </div>
        </div>
        """, unsafe_allow_html=True)
...
# ─────────────────────────────────────────────
    ...
    with tab_e:
        ...
        edited_master = st.data_editor(
            ...
            column_config={
                "大カテゴリ": st.column_config.SelectboxColumn("大カテゴリ", options=CATEGORIES, required=True),
                "製品名": st.column_config.TextColumn("製品名", required=True),
                "初期在庫数": st.column_config.NumberColumn("初期在庫数 (ケース)", min_value=0, required=True),
            }
        )
        if st.button("💾 製品マスタを保存", type="primary"):
            save_data("master", edited_master)
    ...
    with tab_a:
        ...
        with c1:
            new_cat = st.selectbox("大カテゴリ ＊", [""] + CATEGORIES)
        with c2:
            new_product = st.text_input("製品名 ＊")
        with c3:
            new_inv = st.number_input("初期在庫数", min_value=0, value=0, step=10)
        if st.button("➕ 追加する", type="primary"):
            if not new_cat or not new_product.strip():
                st.error("⚠️ カテゴリと製品名は必須です。")
            else:
                latest_master = load_master()
                save_data("master", pd.concat([
                    latest_master,
                    pd.DataFrame([{"大カテゴリ": new_cat, "製品名": new_product.strip(), "初期在庫数": int(new_inv)}])
                ], ignore_index=True))
    ...
    with tab_d:
        ...
        if del_product and st.button("🗑️ 削除する", type="primary"):
            latest_master = load_master()
            save_data("master", latest_master[~((latest_master["大カテゴリ"] == del_cat) & (latest_master["製品名"] == del_product))])
            st.toast("🗑️ 削除しました", icon="✅")
            st.rerun()
        st.markdown("</div>", unsafe_allow_html=True)
    ...
    with tab_c:
        ...
        edited_cust = st.data_editor(
            cust_df, use_container_width=True, num_rows="dynamic",
            column_config={
...
# ─────────────────────────────────────────────
    ...
    <div class="card">
      ...
      <ol>
        <li>左メニューから <b>「📋 受注登録 (出庫)」</b> をクリック。</li>
        <li><b>納品予定日</b> と <b>ケース数</b> を入力。</li>
        <li><b>顧客名</b> を選ぶ。<i>（※ひらがな検索可能。新規は「✏️ 新規・直接入力」を選択）</i></li>
        <li><b>大カテゴリ</b>と<b>製品</b> を選ぶ。<i>（※白黒は自動で「⚪️」「⚫️」マークがつきます）</i></li>
        <li><b>「✅ 受注を登録する」</b> ボタンを押せば完了！</li>
      </ol>
    </div>
    <div class="card">
      ...
      <ol>
        <li>左メニューから <b>「🏭 製造登録 (入庫)」</b> をクリック。</li>
        <li><b>製造予定日</b> と <b>製造ケース数</b> を入力。（備考も入力可）</li>
        <li><b>大カテゴリ</b> と <b>製品</b> を選び、<b>「➕ 製造を登録する」</b> を押せば完了！</li>
      </ol>
    </div>
    <div class="card">
      <div class="card-title">3. 🚨 欠品しないか確認するには？（在庫確認）</div>
      <ul>
        <li><b>左のメニューの下をチェック！</b><br>在庫が足りなくなる予定が入ると、ここが <b>「⚠️ 欠品アラート」</b> に変わります。</li>
        <li>左メニューの <b>「📦 スケジュール・在庫確認」</b> を開くと、いつ・何が足りないか表で確認できます。</li>
        <li><b>👉 解決方法：</b>足りない製品がわかったら、「🏭 製造登録」から製造予定を入力してください。警告が消えます！</li>
      </ul>
    </div>
    <div class="card">
      <div class="card-title">4. ✏️ 入力間違いの修正や、マスタの登録（棚卸し）</div>
      <ul>
        <li><b>データの修正：</b>各登録画面の上の<b>「✏️ データ一覧・個別編集」</b>タブから直せます。</li>
        <li><b>初期在庫の入力：</b>「⚙️ マスタ管理」の「✏️ 製品一覧・編集」タブで、現在倉庫にある実際の数を入力し<b>保存</b>してください。</li>
        <li><b>新しい顧客のふりがな：</b>「⚙️ マスタ管理」の「🏢 顧客マスタ」タブから登録できます。</li>
      </ul>
    </div>
    """, unsafe_allow_html=True)
