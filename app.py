    ...
    if st.button("✅ 受注を登録", type="primary", use_container_width=True):
        if not prod or not qty or to_int(qty)<1:
            ...
            if iadj:
                ...
                app_sync("orders", pd.DataFrame(# 💡 PART 1の最終行「app_sync("orders", pd.DataFrame(」の直後にそのまま連結してください：
[{"ID":nid,"納品予定日":ddt,"顧客名":cn,"大カテゴリ":cat,"製品名":prod,"ケース数":to_int(qty),"運送会社":sc or "","備考":frem,"荷姿チェック":False,"発送備考":"","不良廃棄フラグ":iirr,"日付未定フラグ":idu,"登録日時": datetime.now(JST).replace(tzinfo=None)}]))
    ...
    if st.button("➕ 製造データを記録", type="primary", use_container_width=True):
        if not pm or not mq: st.error("⚠️ 製品・数量は必須")
            ...
            if iadj_m:
                ...
                _mfg_mat_msg = ""
                if ipl and not mst_u.empty && pm in mst_u["製品名"].values:
