...
if not master_df_unique.empty:
    # 受注(orders): 通常出荷も不良廃棄もすべて「在庫減(マイナス)」として扱う
    ...
    # 製造(manufactures): 通常製造もリパックもすべて「在庫増(プラス)」として扱う
...
if page == "📋 受注登録":
    ...
    with st.container():
        ...
        # 【修正】在庫減の理由を明確化
        is_irregular = col_chk2.checkbox("⚠️ 水漏れ・イレギュラー (不良廃棄)")
    ...
    if not orders_df.empty:
        ...
        edited = st.data_editor(recent[["ID", "納品予定日", "顧客名", "製品名", "ケース数", "運送会社", "備考", "不良廃棄フラグ"]], num_rows="dynamic", use_container_width=True, hide_index=True, column_config={"納品予定日": st.column_config.DateColumn("納品日", format="YYYY-MM-DD"), "ケース数": st.column_config.NumberColumn("ケース数", min_value=1, step=1, format="%d")}, key="edit_o")
...
# --- 🏭 製造登録 ---
    ...
    with st.container():
        ...
        if st.button("➕ 製造データを記録する", type="primary", use_container_width=True):
            if not prod_m or m_qty is None: msg_slot_m_add.error("⚠️ 【製品・数量】は必須です。")
                ...
                rem = "【リパック】" if is_repack else ""
                if not is_pack_link: rem += " 【資材非連動】"
                new_m_id = str(uuid.uuid4())[:6].upper()
                new_row = pd.DataFrame([{
                    "ID": new_m_id, "製造予定日": pd.to_datetime(m_date), "大カテゴリ": cat_m, "製品名": prod_m, 
                    "ケース数": to_int(m_qty), "リパックフラグ": is_repack, "備考": rem.strip(), "登録日時": datetime.now()
                }])
    ...
    if not manus_df.empty:
        ...
        edited_m = st.data_editor(recent_m[["ID", "製造予定日", "製品名", "ケース数", "リパックフラグ", "備考"]], num_rows="dynamic", use_container_width=True, hide_index=True, column_config={"製造予定日": st.column_config.DateColumn("製造日", format="YYYY-MM-DD"), "ケース数": st.column_config.NumberColumn("CS数", min_value=1, step=1, format="%d")}, key="edit_m")
...
# --- 📦 資材管理 ---
    ...
    with t_p1:
        ...
        if not orders_df.empty and not master_df_unique.empty:
            master_pack_info = master_df_unique.set_index("製品名")[["使用資材名", "資材使用数"]].to_dict('index')
            mask = (orders_df["納品予定日"].dt.date >= start_d) & (orders_df["納品予定日"].dt.date <= end_d)
            period_orders = orders_df[mask].copy()
            
            analysis_data = []
            for _, r in period_orders.iterrows():
                prod, qty, rem = str(r.get("製品名", "")), to_int(r.get("ケース数", 0)), str(r.get("備考", ""))
                if prod in master_pack_info and "【資材非連動】" not in rem:
            ...
            if analysis_data:
                ...
                st.write("▼ 製品別・資材別の出荷時消費実績 (自動計算)")
                st.dataframe(pivot_analysis, use_container_width=True)
            else: st.info("指定期間内の出荷連動の実績はありません。")
    ...
    with t_p3:
        ...
        if not pack_log_df.empty:
            ...
            edited_p = st.data_editor(recent_p[["ID", "登録日", "資材名", "処理区分", "数量", "理由"]], num_rows="dynamic", use_container_width=True, hide_index=True, column_config={
                "登録日": st.column_config.DateColumn("登録日", format="YYYY-MM-DD"), 
                "処理区分": st.column_config.SelectboxColumn("処理区分", options=["入庫", "出庫", "製造連動", "出荷連動"]),
...
# --- 📑 登録一覧 ---
    ...
    t_list1, t_list2 = st.tabs(["📋 受注・出荷データ", "🏭 製造データ"])
    ...
    with t_list1:
        if orders_df.empty: st.info("登録データがありません。")
            ...
            st.markdown("""<div style="font-size:14px; margin-bottom:10px;">
                <b>🎨 色：</b> <span style="background-color:#FEF08A; color:#854D0E; padding:2px 6px;">不良廃棄（黄色）</span> / <span style="background-color:#D1FAE5; color:#065F46; padding:2px 6px;">✅ 荷姿完了（緑色）</span>
