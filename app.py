"""
丸実屋 受注・製造・在庫管理アプリ (エラー完全修復・通知位置固定・安定版)
"""
...
if page == "📋 受注登録":
    ...
    with st.container():
        ...
        c_name = c2.selectbox("🏢 顧客名", options=cust_list, index=None, placeholder="空欄（変換して検索）")
        qty = c3.number_input("📦 ケース数", min_value=1, value=None, placeholder="数字を入力...")
    ...
    with t1:
        if not master_df.empty:
            ...
            for _, r in master_df.iterrows():
                ...
                p_future_cumsum = p_future_row.cumsum()
                
                row = {"カテゴリ": r["大カテゴリ"], "製品名": format_name(p), "現在庫": int(curr_stock)}
                for d in dates:
                    row[d.strftime("%m/%d")] = int(curr_stock + p_future_cumsum.get(d, 0))
                inv_list.append(row)
...
# 統計・マスタ
    ...
    if not orders_df.empty:
        ...
        st.plotly_chart(px.bar(orders_df[orders_df["顧客名"]!="未指定"].groupby("顧客名")["ケース_数"].sum().reset_index().sort_values("ケース数", ascending=False).head(15), x="ケース数", y="顧客名", orientation='h', title="主要顧客TOP15"), use_container_width=True)
