"""
丸実屋 受注・製造・在庫管理アプリ (エラー完全修復・通知保証・UI安定版)
"""

import os
# テーマ設定：文字消失を絶対に防ぐための強制設定
...
<style>
    ...
    /* 1. 文字消失を絶対に防ぐためのテキスト指定 */
    html, body, [data-testid="stAppViewContainer"] {
        font-family: 'Noto Sans JP', sans-serif !important;
    }
    p, span, label, div { color: #0F172A !important; font-size: 16px; }
    h1, h2, h3 { color: #1E3A8A !important; }

    /* 2. サイドバー：見やすさ重視 */
    [data-testid="stSidebar"] { background-color: #F8FAFC !important; border-right: 1px solid #E2E8F0; }
    [data-testid="stSidebar"] .stButton > button { height: 50px !important; font-size: 16px !important; font-weight: 700 !important; }

    /* 3. スリムなヘッダー (ご要望通り小さめに) */
    .block-container { padding-top: 1rem !important; }
    .slim-header {
        background: linear-gradient(135deg, #1E3A8A 0%, #3B82F6 100%);
        padding: 10px 20px; border-radius: 10px; color: white !important; margin-bottom: 15px;
    }
    .slim-header h1 { color: white !important; font-size: 18px !important; font-weight: 800 !important; margin: 0 !important; }
...
def append_and_sync(name, new_row_df):
    ...
    row_list = row_copy.fillna("").astype(str).replace(["nan", "None", "NaT"], "").values[0].tolist()
...
if page == "📋 受注登録":
    ...
    with st.container():
        ...
        c_name = c2.selectbox("🏢 顧客名", options=sorted(cust_df[cust_df["顧客名"].str.strip() != ""]["顧客名"].unique().tolist()) if not cust_df.empty else [], index=None, placeholder="検索...")
        ...
        search_p = sc1.text_input("🔍 製品検索", placeholder="製品名の一部...")
        ...
        if st.button("✅ 受注を登録する", type="primary", use_container_width=True):
            if not prod or not qty: st.error("⚠️ 【製品・ケース数】は必須です")
            else:
                ...
                st.session_state.success_msg = f"✨ 受注登録完了：{prod}"; st.rerun()
    ...
    with st.container():
        ...
        m_qty = col2.number_input("📦 ケース数", min_value=1, value=None, placeholder="数字...")
        ...
        search_p_m = sc1_m.text_input("🔍 製品検索", placeholder="検索...", key="sm")
        ...
        if st.button("➕ 製造を記録する", type="primary", use_container_width=True):
            if not prod_m or not m_qty: st.error("⚠️ 【製品・数量】は必須です")
            else:
                ...
                st.session_state.success_msg = f"✨ 製造登録完了：{prod_m}"; st.rerun()
    ...
    with t1:
        if not master_df.empty:
            o_ev = st.session_state.orders_df[["納品予定日", "製品名", "ケース数"]].rename(columns={"納品予定日":"日付", "ケース数":"qty"})
            o_ev["qty"] = -o_ev["qty"]
            m_ev = st.session_state.manus_df[["製造予定日", "製品名", "ケース数"]].rename(columns={"製造予定日":"日付", "ケース数":"qty"})
            ...
            pivot_ev = future_ev.pivot_table(index="製品名", columns="日付", values="qty", aggfunc="sum").reindex(columns=dates, fill_value=0) if not future_ev.empty else pd.DataFrame(0, index=master_df["製品名"], columns=dates)
            inv_list = []
            for _, r in master_df.iterrows():
                p = r["製品名"]
                # NaN 対策：fillna(0) を徹底し、確実に整数計算を行う
                past_qty = int(past_ev.get(p, 0))
                curr_stock = int(r.get("初期在庫数", 0)) + past_qty
                p_future = pivot_ev.loc[p].cumsum() if p in pivot_ev.index else pd.Series(0, index=dates)
                row = {"カテゴリ": r["大カテゴリ"], "製品名": format_name(p), "現在庫": int(curr_stock)}
                for d in dates:
                    row[d.strftime("%m/%d")] = int(curr_stock + p_future.get(d, 0))
                inv_list.append(row)
    ...
    with t2:
        ...
        for i in range(7):
            ...
            m_txt = "\n".join([f"製: {r['製品名']} ({r['ケース数']}cs)" for _, r in manus_df[manus_df["製造予定日"]==d].iterrows()])
            o_txt = "\n".join([f"出: {r['顧客名']} : {r['製品名']} ({r['ケース数']}cs)" for _, r in orders_df[orders_df["納品予定日"]==d].iterrows()])
        ...
        for i in range(7):
            d = today + timedelta(days=i)
            m_h = "".join([f'<div style="background:#F0FFF4; border-left:4px solid #10B981; padding:6px; margin-bottom:4px; border-radius:4px;"><span style="font-weight:700;">{format_name(r["製品名"])}</span> <span style="float:right; font-weight:900; color:#059669;">{int(r["ケース数"])}cs</span></div>' for _,r in manus_df[manus_df["製造予定日"]==d].iterrows()])
            o_h = "".join([f'<div style="background:#F0F7FF; border-left:4px solid #2563EB; padding:6px; margin-bottom:4px; border-radius:4px;"><span style="font-weight:700;">{r["顧客名"]}: {format_name(r["製品名"])}</span> <span style="float:right; font-weight:900; color:#1D4ED8;">{int(r["ケース数"])}cs</span></div>' for _,r in orders_df[orders_df["納品予定日"]==d].iterrows()])
    ...
    if not orders_df.empty:
        ...
        st.plotly_chart(px.bar(orders_df[orders_df["顧客名"]!="未指定"].groupby("顧客名")["ケース数"].sum().reset_index().sort_values("ケース数", ascending=False).head(15), x="ケース数", y="顧客名", orientation='h', title="得意先TOP15"), use_container_width=True)
